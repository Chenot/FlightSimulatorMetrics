...
density_plot <- ggplot(df_simu_perf, aes(x = simu_global_perf, fill = Scenario)) +
  ...
  theme_pubr() +
  labs(title = "Density Plot of Composite Score by Scenario", x = "Composite Score", y = "Density")
...
p_tlx <- df_simu_perf %>%
  ...
  scale_fill_manual(values = scenario_colors) +
  facet_wrap(~Scenario, scales = "free") +
  labs(title = "Composite Score & Subjective Workload",
       x = "TLX Score",
       y = "Composite Score") +
  theme_pubr() +
...
p_oddball <- df_simu_perf %>%
  ...
  geom_point(colour = "black", shape = 21, size = 4) +  # Black border with fill color
  geom_smooth(method = "lm", se = FALSE, color = "black") +
  stat_cor(method = "pearson", aes(group = NULL), label.x = 2, label.y = 1.6) +  # Add correlation info
  scale_fill_manual(values = scenario_colors) +
  facet_wrap(~Scenario, scales = "free") +
  labs(title = "Composite Score & Objective Workload",
       x = "Oddball Score (miss rate percentage)",
